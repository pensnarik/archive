...
def get_file_instance(filename,
    ...
    if extension in ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'tiff']:
        return FileImageMeta(filename, precompiled_hash, parent_file_hash)
    elif extension in ['tgz', 'zip', 'rar', 'gz', 'xz']:
        return FileArchiveMeta(filename, precompiled_hash, parent_file_hash)
    elif extension in ['mp4', 'wmv', 'flv', 'avi', 'webm']:
        return FileVideoMeta(filename, precompiled_hash, parent_file_hash)
    else:
        return FileMeta(filename, precompiled_hash, parent_file_hash)
...
class FileMeta():
    ...
    def __init__(self, filename, precompiled_hash=None, parent_file_hash=None):
        ...
        self.filetype = 'text'
        self.parent_file_hash = parent_file_hash
